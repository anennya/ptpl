...
export const borrowBook = async (
  ...
  if (member.borrowedBooks.length >= 2) {
    return {
      success: false,
      message: "Member has reached the maximum borrowing limit (2 books)",
    };
  }
  ...
  const records: BorrowRecord[] = borrowRecords
    ? JSON.parse(borrowRecords)
    : [];
  localStorage.setItem(
    "borrowRecords",
    JSON.stringify([...records, borrowRecord]),
  );
...
export const renewBook = async (
  ...
  if (record.renewed) {
    return {
      success: false,
      message: "This book has already been renewed once",
    };
  }
