...
const ManageMemberDetail: React.FC = () => {
  ...
  const handlePayFine = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedMember = await updateMemberFine(id, amount, false);
      if (updatedMember) {
  ...
  const handleWaiveFine = async () => {
    ...
    try {
      const updatedMember = await updateMemberFine(id, member.fines, true);
      if (updatedMember) {
        setMember(updatedMember);
        
        setSuccessMessage('Fines waived successfully!');
        setShowSuccess(true);
        setTimeout(() => setShowSuccess(false), 3000);
      }
    } catch (err) {
      console.error('Error waiving fines:', err);
      setError('Failed to waive fines');
    }
  };
